...
- (void) tableView: (UITableView *) tableView didSelectRowAtIndexPath: (NSIndexPath *) p
{
    ...
    /*
     NSUInteger index = [p indexAtPosition:[p length] - 1];
     NSLog(@"%lu", index);
     */
    NSIndexPath *newIndexPath_Make = [indexPath indexPathByRemovingLastIndex];
    
    NSLog(@"newIndexPath_make = %@", newIndexPath_Make );
    
    
    NSIndexPath *newIndexPath_Year = [indexPath indexPathByAddingIndex: p.row];
    /* NSIndexPath int  *newIndexPath_model = &indexPath ;
     */
	...
	if ([model numberOfRows: newIndexPath] > 0) {
        ...
        NSString *make = [model name: newIndexPath_Make];
        NSString *year = [model name: newIndexPath_Year];
		NSString *car_model = [model name: indexPath];
		NSString *string = [year stringByReplacingOccurrencesOfString: @" " withString: @"_"];
        
        string = [@"/" stringByAppendingString: string];
        string = [car_model stringByAppendingString: string];
        
        string = [@"/" stringByAppendingString: string];
        string = [make stringByAppendingString: string];
        
		string = [@"http://www.cars.com/" stringByAppendingString: string];
